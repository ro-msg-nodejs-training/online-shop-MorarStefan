...
router.put("/", async (req, res) => {
  try {
    ...
    if (updatedProduct !== null) {
      ...
      res
        .status(400)
        .json({ message: "Product with ID: " + req.body._id + " not found." });
    }
  } catch (err) {
    res.status(400).json({ message: err });
  }
    ...
    try {
      ...
      if (product !== null) {
        ...
        res
          .status(400)
          .json({
            message: "Product with ID: " + req.body._id + " not found.",
          });
      }
    } catch (err) {
      res.status(400).json({ message: err });
    ...
    if (product !== null) {
      try {
        ...
        if (stat.isFile()) {
          ...
          res
            .status(400)
            .json({ message: "URL: " + product.imageUrl + " is not a file." });
        }
      } catch (err) {
        res
          .status(400)
          .json({
            message: "File with URL: " + product.imageUrl + " not found.",
          });
      }
    } else {
      res
